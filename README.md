# A股行情可视化服务

一个功能强大的零配置A股股票分析可视化服务，提供实时行情、技术指标计算、公司信息展示和交互式图表生成。
![alt text](image.png)
![alt text](image-1.png)

## ✨ 核心特性

### 📊 数据分析能力
- **6个时间窗口**: T-0, T-3, T-7, T-30, T-90, T-180 多维度分析
- **34个技术指标**: 价格、成交量、估值、风险全覆盖
- **智能数据源**: 自动切换 akShare、新浪财经、腾讯财经等多个数据源
- **模拟数据**: 网络故障时自动生成高质量演示数据

### 🏢 公司信息展示 ✨新增
- **基本信息**: 公司名称、全称、行业分类、所属板块
- **上市信息**: 上市市场、纳入日期、上市日期、成立日期
- **股本结构**: 总股本、流通股本、注册资本
- **管理层**: 董事长信息、公司官网

### 📈 可视化功能
- **交互式图表**: 基于Plotly的动态图表展示
- **HTML报告**: 完整的分析报告生成
- **热力图**: 多股票指标对比分析
- **风险收益散点图**: 投资组合可视化

### 🚀 高级功能
- **指数成分股**: 支持中证100、中证200、沪深300、中证500
- **批量分析**: 同时处理多只股票
- **智能缓存**: 6小时缓存机制，提升响应速度
- **数据源标注**: 明确标注数据来源和可信度

## 🎯 支持的输入格式

### 股票代码
- **单只股票**: `000001`, `600519`
- **多只股票**: `000001,600519,000858`

### 指数名称
- **上证100**: `上证100` 或 `SSE100`
- **中证300**: `中证300` 或 `CSI300`
- **中证200**: `中证200`
- **中证500**: `中证500`

## 快速开始

### 环境要求
- Python 3.8+
- uv (推荐) 或 pip

### 安装依赖
```bash
# 使用uv（推荐）
source .venv/bin/activate
uv pip install -r requirements.txt

# 或使用pip
pip install -r requirements.txt
```

### 启动服务
```bash
python main.py
```

服务将在 `http://localhost:8000` 启动

### 访问界面
- **主页**：http://localhost:8000
- **API文档**：http://localhost:8000/docs
- **健康检查**：http://localhost:8000/api/v1/health

## API接口

### 1. 股票分析
```
GET /api/v1/stocks/{input}
```

**参数说明：**
- `input`：股票代码或指数名称
  - 指数：上证100, SSE100, 中证300, CSI300
  - 单个股票：000001
  - 多个股票：000001,600000,000002

**示例：**
```bash
# 分析单个股票
curl "http://localhost:8000/api/v1/stocks/000001"

# 分析指数
curl "http://localhost:8000/api/v1/stocks/CSI300"

# 分析多个股票
curl "http://localhost:8000/api/v1/stocks/000001,600000,000002"
```

### 2. 获取股票信息
```
GET /api/v1/stocks/{input}/info
```

### 3. 批量分析
```
POST /api/v1/batch
```

**请求体：**
```json
{
  "inputs": ["000001", "600000", "CSI300"],
  "force_refresh": false
}
```

### 4. 支持指标
```
GET /api/v1/indicators
```

### 5. 缓存管理
```
GET /api/v1/cache/status    # 缓存状态
GET /api/v1/cache/clear     # 清理缓存
```

### 6. 导出数据
```
GET /api/v1/export/{format}/{input}
```

## 技术架构

### 核心模块
```
src/
├── config.py         # 配置管理
├── data_fetcher.py   # 数据获取（akShare集成）
├── cache.py         # SQLite缓存系统
├── indicators.py    # 技术指标计算
├── analyzer.py      # 核心分析逻辑
├── visualizer.py    # 图表生成
└── api.py          # FastAPI接口
```

### 数据流程
1. **输入解析** → 识别股票/指数模式
2. **数据获取** → akShare API + 缓存策略
3. **指标计算** → 30+技术指标
4. **时间窗口** → 6个时间维度分析
5. **结果输出** → JSON + HTML图表

### 缓存策略
- **缓存时间**：6小时自动过期
- **缓存层级**：基础数据、估值数据、财务数据、分析结果
- **自动清理**：过期数据自动删除

## 使用示例

### Python客户端
```python
import requests

# 分析单个股票
response = requests.get("http://localhost:8000/api/v1/stocks/000001")
data = response.json()

if data['success']:
    print(f"股票数量: {data['data']['stock_count']}")
    print(f"JSON数据: {data['json_url']}")
    print(f"图表: {data['chart_url']}")
```

### JavaScript客户端
```javascript
// 分析股票
fetch('/api/v1/stocks/000001')
  .then(response => response.json())
  .then(data => {
    if (data.success) {
      console.log('分析完成:', data.data.stock_count);
      window.open(data.chart_url, '_blank');
    }
  });
```

### 命令行
```bash
# 查看API文档
curl http://localhost:8000/api/v1/

# 检查服务状态
curl http://localhost:8000/api/v1/health

# 查看支持的指标
curl http://localhost:8000/api/v1/indicators

# 分析中证300指数成分股
curl "http://localhost:8000/api/v1/stocks/CSI300"
```

## 性能特点

- **响应速度**：缓存命中 < 100ms
- **并发支持**：异步处理，支持多用户
- **内存占用**：< 100MB（正常使用）
- **请求限制**：60次/分钟，保护数据源

## 错误处理

服务包含完善的错误处理机制：
- 网络连接异常
- 数据格式错误
- API限流处理
- 股票代码验证

## 扩展功能

### 自定义指标
可在 `src/indicators.py` 中添加自定义技术指标

### 数据源切换
可在 `src/data_fetcher.py` 中配置多个数据源

### 缓存策略
可在 `src/config.py` 中调整缓存时间和大小

## 注意事项

1. **数据源限制**：依赖于免费数据接口，可能存在访问限制
2. **交易日历**：不自动识别节假日，使用自然日计算时间窗口
3. **实时性**：非交易时间获取的是最新交易日数据
4. **数量限制**：指数模式可能包含大量股票，分析时间较长

## 故障排除

### 常见问题

1. **网络连接错误**
   - 检查网络连接
   - 等待后重试
   - 查看缓存状态

2. **服务启动失败**
   - 检查端口8000是否被占用
   - 确认依赖包完整安装
   - 查看日志输出

3. **数据获取失败**
   - 检查股票代码格式
   - 确认股票存在
   - 查看akShare接口状态

### 日志查看
服务启动后会显示详细日志，包括：
- 数据获取状态
- 缓存命中情况
- 请求处理时间
- 错误信息

## 开发指南

### 添加新功能
1. 在对应模块中实现功能
2. 在API接口中暴露
3. 更新文档
4. 编写测试

### 代码风格
- 使用类型提示
- 添加详细注释
- 遵循PEP8规范
- 处理所有异常

## 许可证

MIT License

## 贡献

欢迎提交Issue和Pull Request！

## 更新日志

### v0.1.0
- 初始版本发布
- 支持基础股票分析功能
- 实现技术指标计算
- 提供Web API接口
- 支持交互式图表生成